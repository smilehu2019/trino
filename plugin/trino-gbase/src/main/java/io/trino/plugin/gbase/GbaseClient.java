...
public class GbaseClient
        extends BaseJdbcClient
{
    private static final Logger log = Logger.get(GbaseClient.class);

    private static final int MAX_SUPPORTED_DATE_TIME_PRECISION = 6;
    // GBase driver returns width of timestamp types instead of precision.
    ...
    // GBase driver returns width of time types instead of precision, same as the above timestamp type.
    private static final int ZERO_PRECISION_TIME_COLUMN_SIZE = 8;

    // An empty character means that the table doesn't have a comment in GBase
    ...
    private static final PredicatePushdownController GBASE_CHARACTER_PUSHDOWN = (session, domain) -> {
            ...
            RemoteQueryModifier queryModifier)
    {
        ...
        this.aggregateFunctionRewriter = new AggregateFunctionRewriter<>(
                ...
                ImmutableSet.<AggregateFunctionRule<JdbcExpression, ParameterizedExpression>>builder()
                        ...
                        .add(new ImplementSum(GbaseClient::toTypeHandle))
    ...
    {
        // for GBase, we need to list catalogs instead of schemas
    ...
    public PreparedStatement getPreparedStatement(Connection connection, String sql, Optional<Integer> columnCount)
            throws SQLException
    {
        log.debug("trace getPreparedStatement sql=[%s]", sql);
    ...
    public ResultSet getTables(Connection connection, Optional<String> remoteSchemaName, Optional<String> remoteTableName)
            throws SQLException
    {
        // GBase maps their "database" to SQL catalogs and does not have schemas
    ...
    public Optional<String> getTableComment(ResultSet resultSet)
            throws SQLException
    {
        // Empty remarks means that the table doesn't have a comment in GBase
    ...
    public void setTableComment(ConnectorSession session, JdbcTableHandle handle, Optional<String> comment)
    {
        String sql = format(
                ...
                gbaseVarcharLiteral(comment.orElse(NO_COMMENT))); // An empty character removes the existing comment in GBase
        execute(session, sql);
    }
    ...
    protected String getTableRemoteSchemaName(ResultSet resultSet)
            throws SQLException
    {
        // GBase uses catalogs instead of schemas
    ...
    protected List<String> createTableSqls(RemoteTableName remoteTableName, List<String> columns, ConnectorTableMetadata tableMetadata)
    {
        ...
        return ImmutableList.of(format("CREATE TABLE %s (%s) COMMENT %s", quoted(remoteTableName), join(", ", columns), gbaseVarcharLiteral(tableMetadata.getComment().orElse(NO_COMMENT))));
    }

    private static String gbaseVarcharLiteral(String value)
    {
    ...
    public Optional<ColumnMapping> toColumnMapping(ConnectorSession session, Connection connection, JdbcTypeHandle typeHandle)
    {
        ...
        Optional<ColumnMapping> varcharMapping = switch (jdbcTypeName.toLowerCase(ENGLISH)) {
        ...
        if (varcharMapping.isPresent()) {
            return varcharMapping;
        }
        ...
        Optional<ColumnMapping> jdbcTypeMapping = switch (typeHandle.jdbcType()) {
            ...
            case Types.VARCHAR, Types.NVARCHAR, Types.LONGVARCHAR, Types.LONGNVARCHAR -> Optional.of(defaultVarcharColumnMapping(typeHandle.requiredColumnSize(), false));
    ...
    public static LongReadFunction gbaseDateReadFunctionUsingLocalDate()
    {
    ...
    private LongWriteFunction gbaseDateWriteFunctionUsingLocalDate()
    {
    ...
    private static LongReadFunction gbaseTimestampReadFunction(TimestampType timestampType)
    {
        return new LongReadFunction()
        {
            @Override
    ...
    private static LongReadFunction gbaseTimeReadFunction(TimeType timeType)
    {
        return new LongReadFunction()
        {
            @Override
    ...
    public WriteMapping toWriteMapping(ConnectorSession session, Type type)
    {
        ...
        if (type == DATE) {
            return WriteMapping.longMapping("date", gbaseDateWriteFunctionUsingLocalDate());
        }
    ...
    {
        // GBase doesn't support specifying the catalog name in a rename. By setting the
            ...
            String orderBy = sortItems.stream()
                    .flatMap(sortItem -> {
                        ...
                        return switch (sortItem.sortOrder()) {
                            ...
                            default -> throw new UnsupportedOperationException("Unsupported sort order: " + sortItem.sortOrder());
                        };
                    })
                    .collect(joining(", "));
